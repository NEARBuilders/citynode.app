...
**PARTIAL — gate 5 (per-route lazy over MF) + gate 7 MF half:** remote node-SSR bundles build per-route chunks (`dist/ssr/*.js` — per-route chunks present), emit `remoteEntry.server.js` (the plugin's node recipe requires BOTH `{ target: "node" }` as the plugin's second argument AND the environment named `node`/`mf-ssr`), the host loads exposes over HTTP through the L1 shared instance, and hook-free plugin components render with loader data through MF. **Full React singleton negotiation did not complete on the plain-runtime host**: with `registerShared` provides (thunk format), the container's share replacement breaks module init order (`createRootRoute` undefined / `factory is not a function`). Assessment: harness artifact — #134's rspack-bundled host gets its provides from the build's shared config natively, and its prod smoke (`DEPLOY=true` + `REGRESSION_MODE=prod`) proved container share negotiation in production. Plan 034's host is rspack-bundled; the routeConfig-vs-tree module shape is protocol-identical. RESIDUAL RISK for plan 034: re-prove singleton negotiation on the real bundled host with routeConfig exposes (a bounded first step there; STOP if it fails).
