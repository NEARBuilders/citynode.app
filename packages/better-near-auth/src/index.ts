...
function buildListAccountsResponse(nearAccounts: NearAccount[]): ListAccountsResponseT {
  const activeAccount =
    nearAccounts.find((account) => account.isPrimary) ?? nearAccounts[0] ?? null;
...
export const siwn = (options: SIWNPluginOptions) => {
  ...
  const getRpcUrl = (network: "mainnet" | "testnet"): string | undefined => {
    ...
    if (typeof options.rpcUrl === "object")
      return (options.rpcUrl as DualNetworkConfig<string>)[network];
    ...
    const initPromise = initRelayer(relayerCfg, network, adapter, secret, apiKey, networkRpcUrl)
      .then((state) => {
        ...
        if (state) {
          ...
          console.warn(
            `[siwn] Relayer config provided but not initialized for ${network}. Check BETTER_AUTH_SECRET for ephemeral mode.`,
          );
        }
        return state;
      })
  ...
  const getNear = (network: "mainnet" | "testnet") => {
    ...
    if (networkRpcUrl) {
      return new Near({
        network: { rpcUrl: networkRpcUrl, networkId: network },
  ...
  return {
    ...
    hooks: {
      after: [
        {
          matcher: (context: { path?: string; method?: string }) =>
            context.path === "/auth/session" && context.method === "GET",
    ...
    endpoints: {
          ...
          if (nearAccount.isPrimary) {
            ...
            if (otherNearAccounts.length > 0) {
              await ctx.context.adapter.update({
                model: "nearAccount",
                where: [{ field: "id", operator: "eq", value: otherNearAccounts[0]!.id }],
      ...
      getSiwnProfile: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const profile = await (
            options.getProfile || ((id: AccountId) => defaultGetProfile(id, apiKey))
          )(targetAccountId);
        ...
        async (ctx) => {
          ...
          try {
            ...
            if (existingNearAccount) {
              ...
              const anyNearAccount: NearAccount | null = await ctx.context.adapter.findOne({
                model: "nearAccount",
      ...
      getRelayerInfo: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          try {
            ...
            console.error(
              `[siwn] getRelayerInfo: relayer init failed for ${targetNetwork}: ${message}`,
            );
            return ctx.json({
              enabled: false,
              error: message,
          ...
          const { parentAccount, subAccountAvailable } = resolveParentAccount(
            subAccountCfg,
            parentKey,
            rState,
          );
      ...
      createSubAccount: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          const network = (providedNetwork || nearAccount?.network || primaryNetwork) as
            | "mainnet"
            | "testnet";
          const subAccountCfg = getSubAccountConfig(network);
          ...
          if (parentAccount !== rState?.accountId && !effectiveParentKey) {
            throw new APIError("SERVICE_UNAVAILABLE", {
              message:
                "Sub-account parent differs from relayer account. Provide secrets.parentKey to sign as the parent account.",
              status: 503,
            });
          }
          ...
          if (subAccountCfg?.parentHasFullAccess) {
            const parentPublicKey =
              parentAccount === rState?.accountId
                ? rState.publicKey
          ...
          if (subAccountCfg?.init) {
            const initArgs =
              typeof subAccountCfg.init.args === "function"
                ? subAccountCfg.init.args(txCtx)
                : subAccountCfg.init.args;
            txBuilder = txBuilder.functionCall(
              newAccountId,
              subAccountCfg.init.methodName,
              initArgs,
            );
          }
          ...
          try {
            ...
            if (subAccountCfg?.onRollback) {
              try {
                await subAccountCfg.onRollback(lifecycleCtx);
              } catch {}
            } else {
              try {
          ...
          return ctx.json(
            CreateSubAccountResponse.parse({
              success: true,
              accountId: newAccountId,
              network,
              publicKey,
      ...
      checkSubAccountAvailability: createAuthEndpoint(
        ...
        async (ctx) => {
          ...
          if (!parentAccount || isImplicitAccount(parentAccount)) {
            return ctx.json(
              CheckSubAccountAvailabilityResponse.parse({
                available: false,
                accountId: "",
