...
vi.mock("near-kit", () => {
  ...
  return {
    Near: vi.fn(function (this: any) {
      Object.assign(this, mockNearInstance);
    }),
    generateNonce: vi.fn(() => {
      nonceCounter++;
      const nonce = new Uint8Array(32);
...
vi.mock("./profile.js", () => ({
  defaultGetProfile: vi.fn(() =>
    Promise.resolve({ name: "Test User", description: "A test user" }),
  ),
  getImageUrl: vi.fn(() => "https://example.com/image.png"),
  getNetworkFromAccountId: vi.fn((id: string) => (id.endsWith(".testnet") ? "testnet" : "mainnet")),
}));
...
describe("siwn plugin", () => {
  ...
  describe("relayer config validation", () => {
    it("throws BetterAuthError on hybrid relayer config (network keys + flat security fields)", () => {
      ...
      expect(String(caught)).toMatch(
        /whitelistedContracts|maxGasPerTransaction|maxDepositPerTransaction|cannot mix/,
      );
    });
    ...
    it("throws when whitelistedContracts is not an array of strings", () => {
      expect(() =>
        siwn({
          recipient: MOCK_RECIPIENT,
          relayer: { whitelistedContracts: "example.near" as any },
        }),
      ).toThrow();
    });
  ...
  describe("relayer", () => {
    ...
    it("getRelayerInfo returns { enabled:false, error } when ensureRelayer throws at runtime", async () => {
      ...
      const originalImpl =
        generateSpy.getMockImplementation() ??
        (() => ({
          publicKey: {
            data: new Uint8Array(32).fill(1),
            toString: () => MOCK_GENERATED_PUBLIC_KEY,
          },
          secretKey: MOCK_GENERATED_SECRET_KEY,
          sign: vi.fn(),
          signNep413Message: vi.fn(),
  ...
  describe("sub-account availability validation", () => {
    ...
    it("should reject unauthenticated availability check", async () => {
      ...
      const res = await customFetchImpl(
        "http://localhost/api/auth/near/check-sub-account-availability",
        {
          method: "POST",
    ...
    it("should return not-configured when parentAccount is not set", async () => {
      ...
      const availRes = await customFetchImpl(
        "http://localhost/api/auth/near/check-sub-account-availability",
        {
          method: "POST",
          headers: { "Content-Type": "application/json", cookie },
...
describe("siwnClient getActions", () => {
  ...
  describe("getAccountId session fallback", () => {
    ...
    it("returns the primary nearAccount from session when NearConnect is not connected", () => {
      ...
      sessionAtom.set({
        data: {
          user: {
            id: "user-1",
            nearAccount: { accountId: "bob.near", network: "testnet", isPrimary: true },
          },
        },
      });
    ...
    it("returns null when nearAccount.accountId is not a string", () => {
      const { actions } = setupClient({
        data: { user: { id: "user-1", nearAccount: { accountId: 123 } } },
      });
      expect(actions.near.getAccountId()).toBeNull();
    });
  });
  ...
  describe("setPrimaryAccount", () => {
    it("notifies $sessionSignal so the session's nearAccount refreshes", async () => {
      ...
      expect(store.notify as unknown as ReturnType<typeof vi.fn>).toHaveBeenCalledWith(
        "$sessionSignal",
      );
      expect(
        plugin
          .getAtoms(undefined as unknown as Parameters<typeof plugin.getAtoms>[0])
          .nearState.get(),
      ).toEqual({
        accountId: "bob.near",
        publicKey: "ed25519:bob",
  ...
  describe("ml-dsa-65 signatures", () => {
    ...
    async function makeMlDsa65VerifyBody(
