...
async function main() {
  ...
  runChecks("TENANT CONFIG — swapped dashboard UI, shared dashboard API", [
    ...
    [
      ...
      `GET /dashboard → leaf "${dash.leaf}" (same namespaced id as base)`,
    ],
    [
    ...
    [
      revenue.leaf.includes("dashboard__public"),
      `GET /dashboard/revenue matches (tenant-only route exists): "${revenue.leaf}"`,
    ],
    ...
    [
      ...
      `shared apiClient.dashboard.getStats() → ${stats.users} users / ${stats.projects} projects`,
    ],
  ]);
}
